...
def CASE(statement, Items, Items_lines, line_list):
    return CaseStatement(statement, Items, Items_lines, line_list)
...
class VerilogParser:
    ...
    def Token_obj_Replace(self, NewToken):
        for key_in in self.Module_Node.inputs:
            NewToken = re.sub(r"(?<!([a-z]|[A-Z]|_))" + key_in + r"(?!([0-9]|_))",
                              "self.Module_Node.inputs[\"" + key_in + "\"]", NewToken)
        for key_int in self.Module_Node.internal:
            NewToken = re.sub(r"(?<!([a-z]|[A-Z]|_))" + key_int + r"(?!([0-9]|_))",
                              "self.Module_Node.internal[\"" + key_int + "\"]", NewToken)
        for key_out in self.Module_Node.outputs:
            NewToken = re.sub(r"(?<!([a-z]|[A-Z]|_))" + key_out + r"(?!([0-9]|_))",
