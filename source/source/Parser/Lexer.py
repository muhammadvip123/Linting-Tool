...
class Lexer:
    def __init__(self, file):
        ...
        self.Token = Tokens()
        self.assign_id = 0
        self.inputs, _, _, self.outputs, _, _, self.internals, _, _ = get_in_out_signal_names(file)
    ...
    def prep_line(self, line) -> list:
        ...
        for inp in self.inputs:
            line = re.sub(inp + r"(?!([0-9]|_))", " " + inp + " ", line)
        for out in self.outputs:
            line = re.sub(out + r"(?!([0-9]|_))", " " + out + " ", line)
        for internal in self.internals:
            line = re.sub(internal + r"(?!([0-9]|_))", " " + internal + " ", line)
        newToken = line.split(" ")
    ...
    def if_lexer(self, newToken):
        ...
        # print(line)
        condition = re.search(r"if\(.*\)", line).group()
        condition = re.sub(r"if", "", condition)
    ...
    def Case_lexing(self, newToken):
        ...
        case_statement = re.sub(r"(case|\(|\)|\s|\t)", "", line)
    ...
    def doLexing(self):
        ...
        while self.current_line < len(self.lines):
            ...
            if assign:
                ...
                print(newToken)
                self.Token.insert_token(newToken, "assign " + str(self.assign_id), self.get_line_position(self.lines[self.current_line-1]))
                ...
                if end:
                    ...
                    if newToken[-1] == ",":
                        newToken = newToken[:-1]
                    newToken += "],[" + str(start_line) + "," + str(self.get_line_position(self.lines[self.current_line-1])) + "])"
                    self.Token.insert_token(newToken, "always", [start_line, self.get_line_position(self.lines[self.current_line-1])])
                    print([newToken, "always", [start_line, self.current_line]])
                elif case:
