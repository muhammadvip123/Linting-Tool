    ...
    def check_non_full_parallel_case(self, Recursiveobject):
        if isinstance(Recursiveobject, VerilogModule):
            ...
            if "default" in Recursiveobject.items:
                ...
                if 2 ** Recursiveobject.statement.size == len(Recursiveobject.items):
                    Full_case = 1
                if not Full_case:
        ...
        if isinstance(Recursiveobject, VerilogModule):
            ...
            for diff in difference:
                if diff in ParentLHS:
                    ...
                    if diff in self.error_panel["variable Impacted"][6]:
                        position = self.error_panel["variable Impacted"][6].index(diff)
                        for ind in index:
                            self.error_panel["Error line"][6][position].append(ind)
                    else:
                        self.error_panel["Num errors"][6] += 1
                        self.error_panel["Error line"][6].append(index)
                        self.error_panel["variable Impacted"][6].append(diff)
            for Assignment in Recursiveobject.TrueStatements:
                if isinstance(Assignment, ContinuousAssignment):
                    pass
                elif isinstance(Assignment, IfCondition):
